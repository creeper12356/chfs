...
#define LOGIC_2_RAW(i) (i - 1)
  ...
  auto InodeManager::set_table(inode_id_t idx, block_id_t bid) -> ChfsNullResult
  {
    ...
    // whose index is `idx`.
    auto inode_per_block = bm->block_size() / sizeof(block_id_t);
    block_id_t inode_block_id = 1 + idx / inode_per_block;
    usize offset = idx % inode_per_block;

    bm->write_partial_block(inode_block_id, reinterpret_cast<u8 *>(&bid), offset * sizeof(block_id_t), sizeof(block_id_t));
  ...
  auto InodeManager::get(inode_id_t id) -> ChfsResult<block_id_t>
  {
    ...
    // table index.
    auto inode_per_block = bm->block_size() / sizeof(block_id_t);
    block_id_t inode_block_id = 1 + LOGIC_2_RAW(id) / inode_per_block;

    std::vector<u8> buffer(bm->block_size());
    usize offset = LOGIC_2_RAW(id) % inode_per_block;
    bm->read_block(inode_block_id, buffer.data());

    res_block_id = *reinterpret_cast<block_id_t *>(buffer.data() + offset * sizeof(block_id_t));
  ...
  auto InodeManager::free_inode(inode_id_t id) -> ChfsNullResult
  {
    ...
    this->set_table(LOGIC_2_RAW(id), KInvalidBlockID);

    auto bits_per_block = bm->block_size() * KBitsPerByte;
    block_id_t inode_bitmap_block_id = 1 + n_table_blocks + (id) / bits_per_block;
    usize inode_bitmap_idx = LOGIC_2_RAW(id) % bits_per_block;

    std::vector<u8> buffer(bm->block_size());
    bm->read_block(inode_bitmap_block_id, buffer.data());
    auto bitmap = Bitmap(buffer.data(), bm->block_size());
    if (!bitmap.check(inode_bitmap_idx))
    {
      // 该inode已经被释放
      return ChfsNullResult(ErrorType::INVALID_ARG);
    }
